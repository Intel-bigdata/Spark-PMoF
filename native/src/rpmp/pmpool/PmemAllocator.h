...
#ifndef PMPOOL_PMEMOBJALLOCATOR_H_
#define PMPOOL_PMEMOBJALLOCATOR_H_
...
class PmemObjAllocator : public Allocator {
 public:
  explicit PmemObjAllocator(DiskInfo *diskInfos, NetworkServer *server, int wid)
      : diskInfo_(diskInfos), server_(server), wid_(wid) {}
  ...
  int init() override {
    ...
    if (create()) {
      int res = open();
      if (res) {
        std::cout << "failed to open pmem pool, errmsg: " << pmemobj_errormsg()
                  << std::endl;
      }
    }
    return 0;
 ...
 private:
  ...
  std::mutex mtx;
  std::unordered_map<uint64_t, PMEMoid> index_map;
  uint64_t total = 0;
  char str[1048576];
  Chunk *base_ck;
};

#endif  // PMPOOL_PMEMOBJALLOCATOR_H_
