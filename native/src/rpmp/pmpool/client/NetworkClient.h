...
class NetworkClient : public RmaBufferRegister {
 public:
  ...
  NetworkClient(string remote_address, string remote_port, int worker_num = 1,
                int buffer_num_per_con = 32, int buffer_size = 65536,
                int init_buffer_num = 64);
  ~NetworkClient();
  ...
  uint64_t get_dram_buffer(char *data, uint64_t size);
