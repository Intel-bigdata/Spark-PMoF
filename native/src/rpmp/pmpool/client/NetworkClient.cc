...
void RequestHandler::addTask(Request *request) {
...
void ClientRecvCallback::operator()(void *param_1, void *param_2) {
  ...
  // std::cout << "ck id " << mid << " buffer id " << ck->buffer_id << " rpmp ck
  // "
  //           << (uint64_t)ck << " con "
  //           << (uint64_t) reinterpret_cast<Connection *>(ck->con)
  //           << " thread id " << std::this_thread::get_id() << " bufmgr "
  //           << (uint64_t)chunkMgr_ << std::endl;
  // test start
...
NetworkClient::NetworkClient(string remote_address, string remote_port,
                             int worker_num, int buffer_num_per_con,
                             int buffer_size, int init_buffer_num)
...
uint64_t NetworkClient::get_dram_buffer(char *data, uint64_t size) {
