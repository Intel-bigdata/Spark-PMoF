#ifndef RPMP_DATASERVICE_REQUESTHANDLER_H
#define RPMP_DATASERVICE_REQUESTHANDLER_H
...
class DataServiceRequestHandler : public ThreadWrapper {
 public:
  ...
  void addTask(std::shared_ptr<DataServiceRequest> request) {
  ...
  int entry() override {
    std::shared_ptr<DataServiceRequest> request;
  ...
  void abort() override {}
  void notify(std::shared_ptr<DataServiceRequestReply> requestReply) {
  ...
  DataServiceRequestReplyContext get(
      std::shared_ptr<DataServiceRequest> request) {
    ...
    while (!ctx->cv_reply.wait_for(lk, 5ms, [ctx, request] {
      ...
      if (elapse > 10s) {
        ...
        fprintf(
            ...
            request->dataServiceRequestContext_.type,
 ...
 private:
  ...
  BlockingConcurrentQueue<std::shared_ptr<DataServiceRequest>> pendingRequestQueue_;
  ...
  struct InflightProxyRequestContext {
    ...
    DataServiceRequestReplyContext requestReplyContext;
    DataServiceRequestReplyContext &get_rrc() { return requestReplyContext; }
  };
  ...
  inflight_insert_or_get(std::shared_ptr<DataServiceRequest> request) {
    const std::lock_guard<std::mutex> lock(inflight_mtx_);
    auto rid = request->dataServiceRequestContext_.rid;
  ...
  void inflight_erase(std::shared_ptr<DataServiceRequest> request) {
    const std::lock_guard<std::mutex> lock(inflight_mtx_);
    inflight_.erase(request->dataServiceRequestContext_.rid);
  }

  void handleRequest(std::shared_ptr<DataServiceRequest> request) {
    auto ctx = inflight_insert_or_get(request);
    DataServiceOpType rt = request->get_rc().type;
    request->encode();
...
#endif //RPMP_DATASERVICE_REQUESTHANDLER_H