...
[[]{#OLE_LINK48 .anchor}]{#OLE_LINK47 .anchor}ww08.2019. Please refer to
PMEMM snapshot for more details.
...
switch(config)\# interface Et(num\_of\_port)/1
...
Make sure the that the field link\_layer is “Ethernet” with below
commend:
...
Project\_root\_path is HPNL folder’s path.
```bash
...
val resultLocation = "[[]{#OLE_LINK11 .anchor}]{#OLE_LINK10
    .anchor}tpcds_1T_result" // place to write results
...
to check the result at spark history server. []{#OLE_LINK26
.anchor}(Need to start history server by
